...
internal class XMLParser : DefaultHandler() {
        ...
        if (rssItem != null) {
            when (localName.toLowerCase()) {
                ...
                LINK -> if (elementValue?.isNotEmpty() == true) {
                    ...
                    if (!isItem) {
                        headerLink = link.toString()
                    }
                }
                IMAGE, URL -> if (elementValue != null && elementValue?.isNotEmpty() == true) {
                    image = elementValue
                    if (!isItem) {
                        headerImage = image.toString()
                    }
                }
                PUBLISH_DATE -> {
                    date = elementValue
                    if (!isItem) {
                        headerDate = date.toString()
                    }
                }
                DESCRIPTION -> {
            ...
            when (localName.toLowerCase()) {
                TITLE -> {
                    headerTitle = elementValue.toString()
                }
                PUBLISH_DATE -> {
                    headerDate = elementValue.toString()
                }
                IMAGE, URL -> {
                    headerImage = elementValue.toString()
                }
                LINK -> {
                    headerLink = elementValue.toString()
                }
                LAST_DATE -> {
                    headerDate = elementValue.toString()
                }
    ...
    companion object {
        ...
        private const val LAST_DATE = "lastbuilddate"
    }
}