...
import com.bcasekuritas.mybest.databinding.DialogRemoveDeviceBinding
...
@FragmentScoped
@AndroidEntryPoint
class DialogRemoveTrustedDevice(private val fromLayout: String) : BaseBottomSheet<DialogRemoveDeviceBinding>(){

    @FragmentScoped
    override val binding: DialogRemoveDeviceBinding by autoCleaned { (DialogRemoveDeviceBinding.inflate(layoutInflater)) }
    ...
    private fun sendCallback(){
        val result = Bundle()
        result.putString(fromLayout, NavKeys.CONST_RES_MANAGED_DEVICE)
        result.putString(NavKeys.CONST_RES_MANAGED_DEVICE, "RESULT_OK")
